            ...
            @if ($order->payments->isNotEmpty())
                <div class="bg-[#14151C] border border-white/10 rounded p-6">
                    ...
                    @foreach ($order->payments as $payment)
                        <div class="font-[IBM_Plex_Mono] text-xs space-y-1">
                            <p class="text-[#9C9788]">Gateway: <span class="text-[#F6F3EC]">{{ ucfirst($payment->gateway) }}</span></p>
                            <p class="text-[#9C9788]">Statut: <span class="{{ $payment->status === 'completed' ? 'text-green-400' : 'text-yellow-400' }}">{{ $payment->status }}</span></p>
                            <p class="text-[#9C9788]">Montant: <span class="text-[#C9A24B]">{{ $payment->amount }} {{ $payment->currency }}</span></p>
                        </div>
                    @endforeach
                </div>
            @endif
