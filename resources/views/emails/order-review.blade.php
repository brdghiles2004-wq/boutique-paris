<!DOCTYPE html>
<html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background:#14151C;font-family:Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0">
<tr><td align="center" style="padding:40px 20px;">
<table width="540" cellpadding="0" cellspacing="0" style="background:#1C1E27;border-radius:16px;overflow:hidden;border:1px solid #2A2C36;">

    <tr><td style="background:#C9A24B;padding:24px 32px;">
        <p style="color:#14151C;font-size:11px;letter-spacing:3px;text-transform:uppercase;margin:0;font-family:monospace;">Boutique Paris</p>
    </td></tr>

    <tr><td style="padding:32px;text-align:center;">
        <div style="font-size:48px;margin-bottom:16px;">⭐</div>
        <h1 style="color:#F6F3EC;font-size:22px;margin:0 0 12px;font-weight:normal;font-family:Georgia,serif;">
            Votre avis nous intéresse !
        </h1>
        <p style="color:#9C9788;font-size:13px;margin:0 0 8px;">
            Bonjour {{ $order->shipping_name }},
        </p>
        <p style="color:#F6F3EC;font-size:13px;line-height:1.7;margin:0 0 28px;">
            Votre commande <strong style="color:#C9A24B;">{{ $order->order_number }}</strong> a été livrée.<br>
            Nous espérons que vous êtes satisfait(e) de votre achat.
        </p>

        <a href="{{ config('app.url') }}/support?subject=Avis+commande+{{ $order->order_number }}"
           style="display:inline-block;background:#C9A24B;color:#14151C;font-family:monospace;font-size:11px;font-weight:bold;letter-spacing:3px;text-transform:uppercase;text-decoration:none;padding:14px 32px;border-radius:8px;">
            ✍ Laisser un avis
        </a>

        <p style="color:#9C9788;font-size:11px;margin:24px 0 0;">
            Merci de nous faire confiance — Équipe Boutique Paris
        </p>
    </td></tr>

    <tr><td style="padding:16px 32px;border-top:1px solid #2A2C36;text-align:center;">
        <p style="color:#9C9788;font-size:10px;font-family:monospace;margin:0;">© {{ date('Y') }} Boutique Paris</p>
    </td></tr>
</table>
</td></tr>
</table>
</body></html>